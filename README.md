...
Device discovery seeks the first device ID and then each ID greater than the previous one, using existing indexes. It needs no schema migration and retains stopped devices. History requests emit a `history_read_usage` log with total `rowsRead`, `deviceRowsRead`, `queries`, and `metadataAvailable`. Counts come from D1 result metadata; if metadata is absent the flag is false and totals are incomplete. Logs contain no device identifiers or readings. Use these logs to compare production/preview read costs; the measured old DISTINCT discovery query read 9,263 rows. Local SQLite query-plan tests verify indexed seeks but cannot establish D1 billed read counts.

History polling is limited to one attempt per five minutes per page, skips hidden tabs and avoids overlapping requests. Returning to a visible page fetches only when due. Failed attempts also wait until the next interval, limiting retries. Freshness labels continue to update without database reads. Live readings, weather polling and the scheduled collector are unchanged.
